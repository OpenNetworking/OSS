...
class GetLicenseTest(TestCase):

    def setUp(self):
        self.url = '/base/v1/license'
        self.sample_license = {
            '1': {
                'Total Amount': '2000.00000000',
                'address': '1H2jo3AQLoNhKXjDUhYTPHE986iBzggfQJ'
            },
            '2': {
                'Total Amount': '85.00000000',
                'address': '12Ef23un7RLY4DRZSejoSM1zqLuwrMX6gw'
            }
        }

    @mock.patch('base.v1.views.get_rpc_connection')
    def test_get_license(self, mock_rpc):
        mock_rpc().getlicenselist.return_value = self.sample_license

        response = self.client.get(self.url)
        self.assertEqual(response.status_code, httplib.OK)
        self.assertEqual(response.json()['1']['issued'], '2000.00000000')
        self.assertEqual(response.json()['1']['address'], '1H2jo3AQLoNhKXjDUhYTPHE986iBzggfQJ')
        self.assertEqual(response.json()['2']['issued'], '85.00000000')
        self.assertEqual(response.json()['2']['address'], '12Ef23un7RLY4DRZSejoSM1zqLuwrMX6gw')
