...
class TestSetUp:
    ...
    def create_block(self, hash, time, height, prev_block, tx_count, in_longest):
        if prev_block:
            return Block.objects.create(hash=hash, time=time, height=height, prev_block=prev_block, tx_count=tx_count,
                                        in_longest=in_longest)
        else:
            return Block.objects.create(hash=hash, time=time, height=height, tx_count=tx_count, in_longest=in_longest)

    def create_coinbase_tx(self, hash, block):
    ...
    def create_mint_tx(self, hash, block, color):
    ...
    def create_normal_tx(self, hash, block, txout, color):
        tx = Tx.objects.create(hash=hash, block=block, version=1, type=0)
        TxIn.objects.create(tx=tx, txout=txout, position=0)
        TxOut.objects.create(tx=tx, value=100, position=1, scriptpubkey=binascii.unhexlify('aaaa'),
    ...
    def create_other_type_tx(self, hash, block, type, color):
        tx = Tx.objects.create(hash=hash, block=block, version=1, type=type)
        TxIn.objects.create(tx=tx, position=0)
...
class GetBlockByHashTest(TestCase):
    def setUp(self):
        test_sample = TestSetUp()
        block1 = test_sample.create_block('000004e0223a146664188edebf7efbce82c3a421ce70f30b71c156368c21caaf', 0, 1,
                                          None, 1, 1)
        block2 = test_sample.create_block('00000319eb1fbe75c75e6ad3970855ac67f8687febd230b3c26c074474889d3b', 1, 2,
                                          block1, 1, 1)
        block3 = test_sample.create_block('0000020897789853ddfa697e3c5b729c34ba10cb722f10147e13e6a0249038dd', 2, 3,
                                          block2, 1, 1)
...
class GetBlockByHeightTest(TestCase):
    def setUp(self):
        ...
        block3 = test_sample.create_block('0000020897789853ddfa697e3c5b729c34ba10cb722f10147e13e6a0249038dd', 2, 3,
                                          block2, 1, 1)
        block3 = test_sample.create_block('00000808ca8cb51a1c380a972ed2394b66cb9fe814fa898374858291c525d399', 3, 2,
                                          block2, 1, 0)
...
class GetColorTxsTest(TestCase):
    def setUp(self):
        ...
        self.txs = []
        for i in range(30):
            self.txs.append(test_sample.create_mint_tx(str(i), self.block1, 1))
        for i in range(30):
            self.txs.append(test_sample.create_mint_tx(str(i + 30), self.block1, 2))
        for i in range(30):
            self.txs.append(test_sample.create_normal_tx(str(i + 60), self.block1, self.txs[i].tx_outs.all()[0], 1))
        for i in range(30):
            self.txs.append(test_sample.create_normal_tx(str(i + 90), self.block2, self.txs[i + 30].tx_outs.all()[0], 2))
        for i in range(30):
            self.txs.append(test_sample.create_other_type_tx(str(i + 120), self.block2, 2, 1))
        for i in range(30):
            self.txs.append(test_sample.create_other_type_tx(str(i + 150), self.block2, 2, 2))
    ...
    def test_get_color_txs(self):
        ...
        # first page
        url = '/explorer/v1/transactions/color/1?page=1'
        response = self.client.get(url)
        self.assertEqual(response.status_code, httplib.OK)
        self.assertEqual(int(response.json()['page']), 1)
        self.assertEqual(len(response.json()['txs']), 50)
        for i in range(50):
            self.assertEqual(int(response.json()['txs'][i]['vouts'][0]['color']), 1)
            self.assertTrue(response.json()['txs'][i]['type'] == 'NORMAL' or
        ...
        # second page
        url = '/explorer/v1/transactions/color/1?page=2'
        response = self.client.get(url)
        self.assertEqual(response.status_code, httplib.OK)
        self.assertEqual(int(response.json()['page']), 2)
